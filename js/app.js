...
window.onerror = function (message, source, lineno, colno, error) {
  ...
  if (!globalErrorShown && typeof Utils !== "undefined" && Utils.showToast) {
    globalErrorShown = true;
    Utils.showToast("Something went wrong. Please refresh the page.", "danger");
    // Reset after 5 seconds to allow new errors to show
    setTimeout(() => {
...
window.addEventListener("unhandledrejection", function (event) {
  ...
  if (event.reason && !String(event.reason).includes("Failed to fetch")) {
    if (!globalErrorShown && typeof Utils !== "undefined" && Utils.showToast) {
      globalErrorShown = true;
...
const app = {
  ...
  init() {
    ...
    try {
      ...
      if (typeof Views === "undefined") {
        throw new Error("Views module not loaded");
      }
      if (typeof Router === "undefined") {
        throw new Error("Router module not loaded");
      }
      if (typeof Charts === "undefined") {
        throw new Error("Charts module not loaded");
      }
      ...
      if (typeof Storage.updateNotificationBadge === "function") {
  ...
  saveTask(e) {
    ...
    const taskData = {
      ...
      estimatedTime: formData.get("estimatedTime")
        ? parseFloat(formData.get("estimatedTime"))
        : null,
  ...
  resetData() {
    if (
      confirm("Abdicate the throne? This clears ALL records irrecoverably.")
    ) {
      Storage.clear();
      location.reload();
    }
            ...
            <div class="modal-body">
                <form onsubmit="app.updateCourse(event, ${course.id})">
                    <div class="form-group">
                        ...
                        <input type="text" name="title" class="form-input" value="${Utils.sanitize(
                          course.title
                        )}" required>
                    </div>
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" name="platform" class="form-input" value="${Utils.sanitize(
                              course.platform || ""
                            )}" placeholder="Coursera, Udemy...">
                        </div>
                        <div class="form-group">
                            <label class="form-label">Duration</label>
                            <input type="text" name="duration" class="form-input" value="${Utils.sanitize(
                              course.duration || ""
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="number" name="dailyGoal" class="form-input" value="${
                              course.dailyGoal || 60
                            }" min="15" max="480">
                        </div>
                        <div class="form-group">
                            <label class="form-label">End Date</label>
                            <input type="date" name="endDate" class="form-input" value="${
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="pledge" class="form-input" rows="2" placeholder="Why are you learning this?">${Utils.sanitize(
                          course.pledge || ""
                        )}</textarea>
                    </div>
                    <div class="btn-row" style="margin-top: 1.5rem;">
                        <button type="button" class="btn btn-outline" onclick="app.deleteCourse(${
                          course.id
  ...
  saveLog(e) {
    ...
    Utils.showToast(
      `Learning logged! Streak: ${streak.current} day${
        streak.current !== 1 ? "s" : ""
      } 🔥`,
      "gold"
    );
  ...
  completeCourse(courseId) {
    if (!confirm("Mark this course as completed? This is a great achievement!"))
      return;

    Storage.completeCourse(this.data, courseId);
    Utils.showToast(
      "Course Completed! 👑 Upload your certificate to seal the victory.",
      "gold"
    );
  ...
  deleteCourse(courseId) {
    if (!confirm("Delete this course and all its logs? This cannot be undone."))
      return;
  ...
  showImage(src) {
    const w = window.open("");
    w.document.write(
      `<html><head><title>Certificate</title><style>body{margin:0;background:#0a0a0a;display:flex;justify-content:center;align-items:center;min-height:100vh;}</style></head><body><img src="${src}" style="max-width:100%;max-height:100vh;object-fit:contain;"></body></html>`
    );
  },
  ...
  confirmDeleteAccount() {
    if (
      !confirm(
        "⚠️ This will permanently delete your account and all cloud data. This action cannot be undone. Are you sure?"
      )
    ) {
      return;
    }
    ...
    const confirmation = prompt(
      `Type your email "${email}" to confirm deletion:`
    );
    ...
    try {
      ...
      if (headerAvatar && headerInitials) {
        ...
        headerInitials.textContent =
          updatedUser.initials || updatedUser.name?.charAt(0) || "U";
      }
    } catch (error) {
      LoadingComponent.hide();
   ...
   * Light mode is removed. Only king-theme and queen-theme are supported.
   * This function is kept for backward compatibility but does nothing.
   * @param {boolean} isLight - Ignored parameter
   */
  toggleTheme(isLight) {
    // Light mode is disabled - only king-theme and queen-theme are supported
    // Use UI.setRole('king') or UI.setRole('queen') to change themes
    console.log('Light mode is disabled. Use role switching (king/queen) instead.');
  },
   ...
   * Note: Light theme is disabled - only dark mode with king/queen styling
   */
  applySettings() {
    const settings = Storage.getSettings();

    // Light theme is disabled - always use dark mode
    // Remove any lingering light-theme class
    document.body.classList.remove("light-theme");
    
    // Apply role-based theme (king or queen) via UI.initTheme()
    ...
    modalContent.innerHTML = `
            ...
            <div id="idea-history-list">
                ...
                `
                        ...
                        .join("")
                    : '<p style="text-align: center; color: #64748b; padding: 2rem;">No ideas yet. Start capturing your brilliant thoughts!</p>'
                }
            </div>
        `;
    modal.classList.add("active");
    ...
    modalContent.innerHTML = `
            ...
            <div id="good-history-list" style="max-height: 400px; overflow-y: auto;">
                ${
                  goods.length > 0
                    ...
                    <div class="recent-item" style="margin-bottom: 0.75rem;">
                        ...
                        <div class="recent-item-content">
                            <p class="recent-item-title">${Utils.sanitize(
                              good.description.substring(0, 60)
                            )}${good.description.length > 60 ? "..." : ""}</p>
                            <p class="recent-item-date">${Utils.formatTimeAgo(
                              good.date
                            )} • ${good.category}</p>
                        </div>
                ...
                `
                        ...
                        .join("")
                    : '<p style="text-align: center; color: #64748b; padding: 2rem;">No good moments logged yet. Start appreciating the good in your life!</p>'
                }
            </div>
        `;
    modal.classList.add("active");
                  ...
                  lessons.length > 0
                    ...
                    <div class="recent-item" style="margin-bottom: 0.75rem;">
                        ...
                        <div class="recent-item-content">
                            ...
                            )}${lesson.content.length > 80 ? "..." : ""}</p>
                            <p class="recent-item-date">${Utils.formatTimeAgo(
                              lesson.date
                            )}${lesson.source ? " • " + lesson.source : ""}</p>
                        </div>
                    </div>
                `
                        )
                        .join("")
                    : '<p style="text-align: center; color: #64748b; padding: 2rem;">No lessons captured yet. Start learning and growing!</p>'
                }
            </div>
        `;
    modal.classList.add("active");
  ...
  filterChronicle(type) {
    ...
    tabs.forEach((tab) => {
      ...
      if (
        tab.textContent.toLowerCase().includes(type === "all" ? "all" : type)
      ) {
    ...
    document.querySelectorAll(".chronicle-date-group").forEach((group) => {
      const visibleCards = group.querySelectorAll(
        '.chronicle-card[style*="display: flex"]'
      );
      const hiddenCards = group.querySelectorAll(
        '.chronicle-card[style*="display: none"]'
      );
      ...
      if (type === "all") {
        ...
        const hasVisibleCards = Array.from(
          group.querySelectorAll(".chronicle-card")
  ...
  async loadRelationships() {
    ...
    try {
      ...
      // Update stats
      const stats = data.relationships.reduce(
        (acc, r) => {
          acc.total++;
  ...
  filterRelationships(filter, type = "purpose") {
    ...
    if (filter !== "all") {
      if (type === "classification") {
        filtered = window.relationshipsData.relationships.filter(
          (r) => r.classification === filter
        );
  ...
  async saveRelationship(e) {
    ...
    try {
      ...
      Utils.showToast(
        isEdit
          ? "Person updated!"
          : "Person added to your rainy day people! 💜",
        "gold"
      );
      this.closeModal();
  ...
  async toggleRelationshipFavorite(id) {
    try {
      const response = await fetch(
        `${CONFIG.API_URL}/relationships/${id}/favorite`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${CONFIG.getAuthToken()}`,
